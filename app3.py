...
# 모드가 변경되면(특히 대화형 모드로 들어올 때) 채팅 기록 초기화 (원하면 유지 쪽으로 바꿀 수 있음)
...
if is_template_mode:
    ...
    if st.session_state.rag_chain and task_mode is not None:
        # 작업 유형에 따라 자동 질문 프롬프트 구성
        if task_mode == "선택 안함":
            # 이전 자동 결과도 함께 초기화 (원하면 유지해도 됨)
            st.session_state.last_task_mode = None
            st.session_state.last_task_result = None
