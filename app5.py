...
# Streamlit UI
# -------------------------------
st.set_page_config(page_title="과제 도우미 챗봇")
st.title("📚 과제 도우미 챗봇")
...
1. 문서를 1~5개 업로드 합니다. (PDF 또는 TXT)   
2. 작업 모드를 선택 합니다.  
3. 선택한 모드에 맞게 질문 또는 요청을 입력(또는 자동 실행)하면, 문서 내용을 중심으로 AI가 응답합니다.  
        ...
        if st.session_state.rag_chain is None:
            ...
            with st.chat_message("assistant"):
                with st.spinner("생각 중입니다..."):
                    result = st.session_state.rag_chain.invoke({"question": user_msg, "history": history_text})
